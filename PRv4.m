...
[X_train, X_test, Y_train, Y_test] = splitSample(X_all,Y, 0.9, true);
...
LDA_Accuracy_test_PCA = sum(strcmp(Y_test, Y_fit_LDA))/length(Y_test)
...
QDA_Accuracy_test_PCA = sum(strcmp(Y_test, Y_fit_QDA))/length(Y_test)
...
GNB_Accuracy_test_PCA = sum(strcmp(Y_test, Y_fit_GNB))/length(Y_test)
...
Logistic_Accuracy_test_PCA = sum(strcmp(Y_test, Y_fit_Logistic))/length(Y_test)
...
Linear_SVM_Accuracy_test_PCA = sum(strcmp(Y_test, Y_fit_Linear_SVM))/length(Y_test)
...
Quadratic_SVM_Accuracy_test_PCA = sum(strcmp(Y_test, Y_fit_Quadratic_SVM))/length(Y_test)
...
zzzz = fitcdiscr(X_train_norm,Y_train,'DiscrimType','Quadratic')
resubLoss(zzzz)
% Forward Subset Selection - Using cross Validation
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[fsCVfor30,historyCV] = sequentialfs(fun,X_train_norm,Y_train,'cv',tenfoldCVP,'Nf',30,'options',opts);
figure
...
C = confusionmat(Y_test, label_test_FS)
